...
LedStrip = function(spi,led_count){
  this.spi = spi;
  ...
  this.buf = new Buffer(led_count*4);//
...
LedStrip.prototype.refresh = function(){
  ...
  result_length = this.init.length+this.buf.length+this.end.length;
  this.spi.write(result);
}

LedStrip.prototype.flush = function(){
  this.buf.fill(0);
  for(i=0;i<4*this.led_count;i+=4){
